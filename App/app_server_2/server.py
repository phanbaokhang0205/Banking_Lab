...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:@localhost/db_banking2'
            ...
            if user.password == _password and user.is_logged_in == False:
                ...
                try:
                    ...
                    response = requests.post(
                        f'http://{server_ip}:5000/account_sync', json=sync_data)
...
def transfer_api():
    ...
    if from_user and to_user and from_user.balance >= _amount:
        ...
        try:
            ...
            response = requests.post(
                f'http://{server_ip}:5000/balance_sync', json=sync_data)
            if response.status_code == 200:
                return jsonify({'message': 'Transfer successful and synced!'})
        except Exception as e:
            return jsonify({'error': 'Sync failed: ' + str(e)}), 500
...
if __name__ == '__main__':
    ...
    with app.app_context():
        db.create_all()
    app.run(debug=True,host=get_server_ip(), port=5002)
